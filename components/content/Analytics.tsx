...
export function Analytics() {
  ...
  // Get colors from chartConfig for use in charts
  const conversionColor = chartConfig.conversions.color;
  const leadsColor = chartConfig.leads.color;
  ...
  const sourceData = [
    {
      name: "Website",
      value: contacts.filter((c) => c.source === "Website").length || 35,
    },
    {
      name: "Referral",
      value: contacts.filter((c) => c.source === "Referral").length || 25,
    },
    {
      name: "LinkedIn",
      value: contacts.filter((c) => c.source === "LinkedIn").length || 20,
    },
  ...
  const repData = [
    {
      rep: "John Doe",
      deals:
        contacts.filter((c) => c.rep === "John Doe" && c.status === "won")
          .length || 8,
    },
    {
      rep: "Jane Smith",
      deals:
        contacts.filter((c) => c.rep === "Jane Smith" && c.status === "won")
          .length || 12,
    },
    {
      rep: "Mike Johnson",
      deals:
        contacts.filter((c) => c.rep === "Mike Johnson" && c.status === "won")
          .length || 6,
    },
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        ...
        <Card className="col-span-full">
          <CardHeader>
            ...
            <CardDescription>
              Deals closed by each sales representative
            </CardDescription>
          </CardHeader>
          <CardContent>
            <ChartContainer config={chartConfig} className="h-[300px]">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              ...
              {(
                contacts.filter((c) =>
                  ["qualified", "proposal"].includes(c.status),
                ).length * 5000
              ).toLocaleString()}
            </div>
